...
def getseg():
    global bcdf
    filenames = fd.askopenfilenames(filetypes=[("Text files","*.xlsx")])
    Label(root, text = filenames[0]).place(x = 200, y  = 212)
...
def expfile():
    ...
    Label(root, text = name).place(x = 200, y  = 132)
...
# Uploads
Button(root, text = 'Template SPOT', command = gettemp).place(x = 59, y = 170)
Button(root, text = 'Meter Data BCRM', command = getmd).place(x = 30, y = 90)
Button(root, text = 'Exclusion File', command = getexcl).place(x = 65, y = 50)
Button(root, text = 'Segregated Data', command = getseg).place(x = 52, y = 210)
Button(root, text = 'MyCloud Folder', command = expfile).place(x = 52, y = 130)
