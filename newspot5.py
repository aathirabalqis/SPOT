...
def getmd():
    ...
    # output IDs in HES format - to find devstat
    mds = md[['Meter ID']].dropna()
    new_ids = mds['Meter ID'].tolist()
...
def getseg():
    global bcdf
    filenames = fd.askopenfilenames(filetypes=[("Text files","*.xlsx")])
    Label(root, text = filenames[0]).place(x = 185, y  = 242)
...
def getstat():
    ...
    devstat = filenames[0]
    devdf = pd.read_excel(devstat)
    devdf = devdf.rename(columns = {'ID' : 'Meter ID'})
    devdf = devdf[['Meter ID','Device Status']]
...
def genmd():
    ...
    # merge device status with meterdata dataframe
    md = pd.merge(left = md, right = devdf, how = 'left')
    md.loc[(md['Device Status'] != 'Commissioned'), 'Device Status'] = 'Not Commissioned'


    # checks for incomplete register
    md = md[['MR Unit','Portion','Installat.','Meter ID','Reg 01 (kWh)','Reg 09 (kWh)','Reg 11 (kWh)','Reg 51 (kWh)','Reading Status','Avg. Consumption','Device Status']]    
...
def genspot():
    ...
    for i in units:
        ...
        # count not comm and not reporting
        notcomm = val.loc[(val['Device Status'] == 'Not Commissioned') & (val['Reading Status'] != 'VAL')]
        notcomm = notcomm['Device Status'].tolist()
   
        notrep = val.loc[(val['Device Status'] == 'Commissioned') & (val['Reading Status'] != 'VAL')]
        notrep = notrep['Device Status'].tolist()

        
        # reorder columns
        val['Remarks'] = ''
        val = val[['MR Unit','Contract Account','Sequence Number','Portion','Installat.','Meter ID','Address','Reg 01 (kWh)','Reg 09 (kWh)','Reg 11 (kWh)','Reg 51 (kWh)','Remarks','Reading Status', 'Device Status']]
        ...
        textbox.insert(END, '\n')
        textbox.insert(END, '\n')
        textbox.insert(END, '*Meters no Reading: ')
        textbox.insert(END, '\n')
        textbox.insert(END, '- Meters not Connected to Network: ' + str(len(notcomm)) + ' Meters')
        textbox.insert(END, '\n')
        textbox.insert(END, '- Meters not Reporting: ' + str(len(notrep)) + ' Meters')
        textbox.insert(END, '\n')
        textbox.insert(END, '--------------------------------------------')
        textbox.insert(END, '\n')
        textbox.insert(END, '\n')
...
# Uploads
Button(root, text = 'Template SPOT', command = gettemp).place(x = 44, y = 200)
...
Button(root, text = 'Segregated Data', command = getseg).place(x = 37, y = 240)
Button(root, text = 'MyCloud Folder', command = expfile).place(x = 37, y = 120)
Button(root, text = 'Device Status', command = getstat).place(x = 52, y = 160)
...
Label(root, text = 'Path: ').place(x = 145, y  = 242)

textvar = StringVar()
textbox = ScrolledText(root, height = 11, width = 45)
textbox.place(x = 65, y = 330)

Button(root, text = 'Generate Meter Data', command = genmd).place(x = 125, y = 290)
Button(root, text = 'Generate SPOT Files', command = genspot).place(x = 255, y = 290)

Label(root, text = 'SAB').place(x = 470, y = 520)
